...
class Yolo(nn.Module):
    def __init__(self, num_classes, anchor_boxes: torch.tensor):
        ...
        self.scale_heads = nn.ModuleList([DetectorHead([1024, 512, 1024], num_classes, self.anchor_boxes / 8),  # scale3 from last resblock (group)
                                          DetectorHead([512, 256, 512], num_classes, self.anchor_boxes / 16),   # scale2 from 2nd to last resblock (group)
                                          DetectorHead([256, 128, 256], num_classes, self.anchor_boxes / 32)    # scale1 from 3rd to last resblock (group)
                                        ])
        
        # Head Upsampling Layer Inits
        self.upsamples = nn.ModuleList([nn.ConvTranspose2d(1024, 512, kernel_size=1, stride=2),     # upsample from scale3 to scale2
                                       nn.ConvTranspose2d(512, 256, kernel_size=1, stride=2),       # upsample from scale2 to scale1
